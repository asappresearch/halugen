...
class BEGINDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data[index]["label"] == "Not fully attributable":
            target_text = "No"
        elif self.data[index]["label"] == "Generic":
            target_text = "generic"
        elif self.data[index]["label"] == "Fully attributable":
            target_text = "Yes"
        else:
            raise Exception(
                f"Invalid label {self.data[index]['label']} at index {index}"
            )
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(loader, 0)):
            ...
            scores = torch.exp(generated_outputs["scores"][-1][:, 2163]) / (
                ...
                + torch.exp(generated_outputs["scores"][-1][:, 8165])
            )  # Yes, No, generic
...
def calc_metrics(pred_path):
    ...
    pos_prec, pos_recall, pos_f1, _ = precision_recall_fscore_support(
        [l == "Yes" for l in labels],
        [l == "Yes" for l in preds],
        pos_label=True,
        average="binary",
        zero_division=0,
    )
    ...
    pos_prec, pos_recall, pos_f1, _ = precision_recall_fscore_support(
        [l == "generic" for l in labels],
        [l == "generic" for l in preds],
        pos_label=True,
        average="binary",
        zero_division=0,
    )
    logging.critical(
        f" | (generic) p = {pos_prec:.3f}, r = {pos_recall:.3f}, f1  = {pos_f1:.3f}"
    )
...
def run(args):
    ...
    if args.mode == "train":
        ...
        negative_data_list = json.loads(
            open(args.train_negative_path).read()
        )  # [:(835 * 4)]
        for d in negative_data_list:
            d["label"] = "Not fully attributable"
        positive_data_list = json.loads(
            open(args.train_positive_path).read()
        )  # [:(313 * 4)]
        for d in positive_data_list:
            d["label"] = "Fully attributable"
        neutral_data_list = json.loads(
            open(args.train_neutral_path).read()
        )  # [:(81 * 4)]
        for d in neutral_data_list:
            d["label"] = "Generic"
        logging.info(f"positive data length {len(positive_data_list)}")
        logging.info(f"negative data length {len(negative_data_list)}")
        logging.info(f"neutral data length {len(neutral_data_list)}")
        train_data_list = positive_data_list + negative_data_list + neutral_data_list
        random.shuffle(train_data_list)

        training_set = BEGINDataSet(train_data_list, tokenizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        default="models/begin_lr_1e-4",
        help="path to the folder to save (if mode is 'train') / load (if mode is 'test') the model",
    )
    parser.add_argument(
        "--output_path",
        default="predictions/begin_lr_1e-4",
        help="path to the json file for saving predictions",
    )
    parser.add_argument("-n", default=10000, type=int, help="number of data to train.")
